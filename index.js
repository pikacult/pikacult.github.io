...
const PikaDimensions = { width: 353 * 1.2, height: 325 * 1.2 };
...
function draw(offset, loopCount) {
  ...
  let currentPercentage = (loopingPikas - loopCount) / loopingPikas
  context.drawImage(
    image,
    -PikaDimensions.width / 2 - offset/2 + (movementOffset.x * currentPercentage),
    -PikaDimensions.height / 2 - offset/2 + (movementOffset.y * currentPercentage),
    PikaDimensions.width + offset,
    PikaDimensions.height + offset
  );
}
